...
for (cell in unique(proj.all$Cell)){


    message(cell)
    selected <- which(proj.all$Cell == cell)
    proj <- proj.all[proj.all$Cell == cell,]
    ...
    # pruned.regulon.motif <- addMotifScore(pruned.regulon,
    #                                       peaks = rowRanges(peakMatrix.cell),
    #                                       species="human",
    #                                       genome="hg38")
    ...
    saveRDS(regulon_df_full, paste0("OUTPUT/ArchRProject/Epiregulon/regulon_df_full.", cell, ".rds"))
    saveRDS(regulon.w, paste0("OUTPUT/ArchRProject/Epiregulon/regulon.w.", cell, ".rds"))
    saveRDS(score.combine, paste0("OUTPUT/ArchRProject/Epiregulon/score.combine.", cell, ".rds"))


    #########
    pdf(paste0("OUTPUT/ArchRProject/Epiregulon/activity.separate.", cell,".pdf"), width = 9, height = 6)


    plotviolin <- plotActivityViolin(score.combine,
                                     tf = c("SMARCA4","SMARCA2", "AR","FOXA1", "NKX2-1","TEAD1"),
                                     clusters = GeneExpressionMatrix.select$TEST_ARTICLE ,
                                     ncol = 3,
                                     title = cell,
                                     colors = c("gray","red","blue","orange"),
                                     boxplot = TRUE)
    print(plotviolin)


    plotviolin <- plotActivityViolin(score.combine,
                                     tf = c("TCF12","NR3C1","ERG","ASCL1","FOSL1","TEAD1"),
                                     clusters = GeneExpressionMatrix.select$TEST_ARTICLE,
                                     ncol = 3,
                                     title = cell,
