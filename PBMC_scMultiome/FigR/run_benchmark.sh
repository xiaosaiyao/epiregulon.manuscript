...
specification="1,64,/gstore/project/epigen/benchmark/PBMC/FigR,PBMC,FigR,preprocessing,Rosalind,$(date +%Y-%m-%d_%H:%M)"
...
file_path="/gstore/project/epigen/benchmark/PBMC/FigR/slurm-${job_id}.out"
...
job_id=$(echo "$sbatch_output" | grep -o 'Submitted batch job [0-9]*' | awk '{print $NF}')
specification="20,256,/gstore/project/epigen/benchmark/PBMC/FigR,PBMC,FigR,main,Rosalind,$(date +%Y-%m-%d_%H:%M)"
echo "${job_id},${specification}" >> /gstore/project/epigen/benchmark/resource_use.txt
